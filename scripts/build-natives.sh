...
ASSET_DIR="${PROJECT_DIR}/app/src/main/assets/aether"
...
build_hev() {
  ...
  # Running the loop on hev's own pthread (legacy proxy's proven mode) avoids ART
  # entirely.
...
build_aether() {
  ...
  build_aether_abi() {
    ...
    mkdir -p "${JNI_DIR}/${abi}" "${ASSET_DIR}/${abi}"
    cp "${artifact}" "${JNI_DIR}/${abi}/libaether.so"
    "${NDK_TOOLCHAIN}/llvm-strip" "${JNI_DIR}/${abi}/libaether.so" 2>/dev/null || true
    # Keep an explicit executable copy in assets as a packaging fallback.
    # Some Android package/install paths do not expose non-standard native
    # executables through ApplicationInfo.nativeLibraryDir.
    cp "${JNI_DIR}/${abi}/libaether.so" "${ASSET_DIR}/${abi}/libaether.so"
    chmod 0755 "${ASSET_DIR}/${abi}/libaether.so"
    echo "    installed libaether.so for ${abi} (jniLibs + assets fallback)"
  }
...
if [ "${TARGET}" = "aether" ] || [ "${TARGET}" = "all" ]; then
  for abi in "${ABIS[@]}"; do
    test -s "${JNI_DIR}/${abi}/libaether.so" || { echo "ERROR: missing ${JNI_DIR}/${abi}/libaether.so" >&2; exit 1; }
    test -s "${ASSET_DIR}/${abi}/libaether.so" || { echo "ERROR: missing ${ASSET_DIR}/${abi}/libaether.so" >&2; exit 1; }
  done
fi
