...
android {
    ...
    defaultConfig {
        ...
        versionCode = 1
        versionName = "1.0"
        buildConfigField("String", "CORE_VERSION", "\"aetherst-1.6.9\"")
...
tasks.register("verifyAetherEngineAssets") {
    doLast {
        ...
        requiredAbis.forEach { abi ->
            val engine = file("src/main/jniLibs/$abi/libaether.so")
            if (!engine.isFile || engine.length() == 0L) {
                throw GradleException("Missing AetherST engine for $abi: ${engine.absolutePath}")
            }
        }
    }
}
