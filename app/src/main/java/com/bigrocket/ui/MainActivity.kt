...
import com.bigrocket.service.AetherSTConfigStore
...
class MainActivity : AppCompatActivity() {
    ...
    private var aetherConfig by mutableStateOf(AetherConfig())
    ...
    private fun setupUpstreamIntegration() {
        aetherConfig = AetherSTConfigStore.load(this)
        ...
        aetherEmbeddedPanel.setContent {
            Surface {
                AetherEmbeddedPanel(
                    ...
                    onConfigChange = { cfg ->
                        aetherConfig = cfg
                        AetherSTConfigStore.save(this, cfg)
                    },
                    enabled = EmbeddedAetherRuntime.enabled.collectAsState().value,
                    onEnabledChange = { selected ->
                        setUpstreamChoice(if (selected) UpstreamChoice.AETHER else UpstreamChoice.NONE)
                    },
                )
            }
        }

        val options = arrayOf("فقط BigRocket", "BigRocket + AetherST")
        ...
        spinnerAetherMode.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                setUpstreamChoice(if (position == 1) UpstreamChoice.AETHER else UpstreamChoice.NONE)
            }
        }
    ...
    private fun renderUpstreamChoice() {
        tvAetherMode.text = when (upstreamChoice) {
            UpstreamChoice.AETHER -> "مسیر بعد از Bonding: BigRocket → Path 3 → AetherST"
